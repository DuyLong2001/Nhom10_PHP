...
<body>
    ...
    if(isset($_POST['submit'])){
        if(isset($_POST['s1']) and is_numeric($_POST['s1']) and isset($_POST['s2']) and is_numeric($_POST['s2'])){
            $so1=$_POST['s1'];
            $so2=$_POST['s2'];
            ...
            if($_POST["gender"]=='chia'){
                if($so2==0) { $kq='kết quả không hợp lệ ';}
                else $kq=$so1/$so2;
                
            }
        }else{
            $kq='vui lòng nhập đúng số liệu';
        }
    }
    ...
    <table align="center">
        ...
        <tr>
            ...
            <td> <input type="text" name="s1a"
                        value="<?php if(isset($so1)) echo $so1; else echo 'Chưa nhập số hoặc số liệu không đúng';?>" size="35">
            </td>
        </tr>
        <tr>
            <td>Số 2: </td>
            <td> <input type="text" name="s2a"
                        value="<?php if(isset($so2)) echo $so2; else echo 'Chưa nhập số hoặc số liệu không đúng';?>" size="35">
            </td>
        </tr>
        <tr>
            <td>Kết quả: </td>
