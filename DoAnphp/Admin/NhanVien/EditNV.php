...
require('./connect.php');
$id_dm=$_GET['id_dm'];
$query1="SELECT * FROM nhan_vien WHERE MaNV=$id_dm ";
    ...
    if(isset($_POST['submit'])){
      ...
      and empty($_POST['dc'])){
        ...
        $email=$_POST['email'];
        $diachi=$_POST['dc'];
        $query="UPDATE nhan_vien SET HoTenNV='$hoten',TenDN='$tendn'
        ,SDT='$sdt',Email='$email',DiaChi='$diachi' WHERE MaNV=$id_dm";
        $result = mysqli_query($conn,$query);
        if(!$result){
          $thongbao="Cập nhật thất bại vui lòng thực hiện lại";
        }else $thongbao="Cập nhật mới thành công";
      }
                  ...
                  <div class="card-body">
                    ...
                    <form class="forms-sample" method="post">
                      <div class="form-group">
                        <label >Họ Tên</label>
                        <input type="text" class="form-control"  name="ht" placeholder="Họ và tên" value="<?php if(isset($hoten)) echo $hoten; else echo $rows['HoTenNV']?>">
                      </div>
                      <div class="form-group">
                        <label >Tên đăng nhập</label>
                        <input type="text" class="form-control"  name="tdn" placeholder="Username" value="<?php if(isset($tendn)) echo $tendn; else echo $rows['TenDN']?>">
                      </div>
